...
scale = 0.7 # SCALE FOR THE RECTANGLE AREA TO DRAW AFTERWARDS
...
X_train = pd.DataFrame(data = x_train, index = range(34), columns = ["Airq","Area","Humidity","Temperature"])
Y_train = pd.DataFrame(data = y_train, index = range(34), columns= ["NoL"])
X_test = pd.DataFrame(data = result2, index = range(1), columns = ["Airq","Area","Humidity","Temperature"])
...
Y_pred = regression.predict(X_test) # tahmin için internetten çektiğim veriyi girdim
